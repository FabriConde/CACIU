...
class escenaPYGFX:
    ...
    def limpiar_modelos(self):
        # Elimina todos los objetos menos las luces y el fondo
        objetos_a_remover = [obj for obj in self.scene.children if not isinstance(obj, gfx.Light)]
        for obj in objetos_a_remover:
...
class Program:
    def __init__(self, store, update_fn, view_fn):
        self.store = store
    ...
    async def process_updates(self):
        while True:
            ...
            await self.update_fn(self.store, msg)

    async def run_tasks(self):
        await asyncio.gather(*[t(self.store, self.queue) for t in self.tasks])

    async def start(self):
        cap = cv2.VideoCapture(self.store.cam, self.store.bk)
        view_fn = self.view_fn(self.store)

        await asyncio.gather(
            self.process_updates(),
            self.run_tasks(),
            self.stream(cap, view_fn)
        )

    async def stream(self, cap, view_fn):
        while True:
            ret, frame = cap.read()
